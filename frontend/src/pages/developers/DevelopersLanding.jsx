...
const FEATURES = [
	...
	{ icon: <Globe className="w-6 h-6" />, title: "15 pays couverts", desc: "Toute la zone UEMOA, CEMAC, RDC, Gambie, et Guinée Conakry.", color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20" },
...
const MerchantsLanding = () => {
	...
	return (
		<div className="min-h-screen bg-slate-50 dark:bg-secondary-950 font-sans">
			...
			{/* ── Hero ── */}
			<section className="relative overflow-hidden pt-32 pb-24 bg-white dark:bg-slate-900">
				...
				<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[400px] bg-primary-600/10 rounded-full blur-[120px] pointer-events-none" />
				...
				<div className="container mx-auto px-4 max-w-6xl relative z-10">
					<div className="flex flex-col lg:flex-row items-center gap-14">
						...
						<div className="lg:w-1/2 text-center lg:text-left">
							<h1 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white leading-[1.05] tracking-tight mb-6">
								L'API de transfert<br />
								<span className="text-primary-500">
									vers l'Afrique
								</span>
							</h1>
							<p className="text-lg text-slate-600 dark:text-slate-300 mb-10 leading-relaxed max-w-lg mx-auto lg:mx-0">
								Intégrez Wave, Orange Money, MTN Mobile Money et plus encore
								en quelques lignes de code. Couvrant 15 pays d'Afrique.
							</p>
							<div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
								<Link to="/register"
									className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-primary-500 hover:bg-primary-400 text-white font-black rounded-md transition-all hover:-translate-y-0.5 ">
									<Key className="w-5 h-5" /> Obtenir une clé API <ArrowRight className="w-5 h-5" />
								</Link>
								<Link to="/docs"
									className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-900 dark:text-white font-bold rounded-md border border-slate-200 dark:border-white/20 transition-all">
									<BookOpen className="w-5 h-5" /> Documentation
								</Link>
							</div>
							<div className="flex flex-wrap gap-10 mt-12 justify-center lg:justify-start">
								{[{ v: "99.9%", l: "Uptime" }, { v: "<200ms", l: "Latence" }, { v: "15 pays", l: "Couverts" }].map(s => (
									<div key={s.l}>
										<p className="text-2xl font-black text-slate-900 dark:text-white">{s.v}</p>
										<p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">{s.l}</p>
									</div>
								))}
							</div>
						</div>
						...
						<div className="lg:w-1/2 w-full">
							<div className="rounded-md bg-slate-950 border border-slate-700/50 overflow-hidden ">
			...
			<section className="py-24 bg-white dark:bg-secondary-950 border-b border-slate-100 dark:border-secondary-900">
				<div className="container mx-auto px-4 max-w-6xl">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
						{FEATURES.map((f, i) => (
							<div key={i} className="group p-8 rounded-md bg-slate-50 dark:bg-secondary-900/50 border border-slate-100 dark:border-secondary-800 hover: hover:-translate-y-2 transition-all duration-500">
								<div className={`w-12 h-12 rounded-md ${f.color} flex items-center justify-center mb-5 group-hover:scale-110 transition-transform`}>{f.icon}</div>
				...
				<div className="container mx-auto px-4 max-w-6xl">
					<div className="flex flex-col lg:flex-row items-center gap-16">
						...
						<div className="lg:w-1/2 w-full space-y-3">
							{ENDPOINTS.map((ep, i) => (
								<div key={i} className="flex items-center gap-4 p-4 rounded-md bg-slate-50 dark:bg-secondary-900/60 border border-slate-100 dark:border-secondary-800 hover:border-primary-200 dark:hover:border-primary-900/50 transition-colors group">
									<span className={`${ep.color} text-white text-[10px] font-black px-2.5 py-1 rounded shrink-0 font-mono`}>{ep.method}</span>
			...
			<section className="py-24 relative overflow-hidden bg-white dark:bg-slate-900">
				...
				<div className="container mx-auto px-4 max-w-3xl text-center relative z-10">
					<h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-6 tracking-tight">Prêt à intégrer l'API ?</h2>
					<p className="text-slate-600 dark:text-slate-300 text-lg mb-10 font-medium">Créez votre compte Marchand gratuitement. Votre première clé API est disponible immédiatement.</p>
					<div className="flex flex-col sm:flex-row gap-4 justify-center">
						<Link to="/register"
							className="inline-flex items-center justify-center gap-2 px-10 py-4 bg-primary-500 hover:bg-primary-400 text-white font-black rounded-md transition-all hover:-translate-y-0.5 text-lg">
							<Key className="w-5 h-5" /> Créer mon compte gratuit
						</Link>
						<Link to="/contact"
							className="inline-flex items-center justify-center gap-2 px-10 py-4 bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-900 dark:text-white font-bold rounded-md border border-slate-200 dark:border-white/20 transition-all">
							Parler à l'équipe
						</Link>
					</div>
				</div>
