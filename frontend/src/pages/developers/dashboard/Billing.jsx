...
const Billing = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="p-8 rounded-3xl bg-white dark:bg-secondary-900 border border-slate-100 dark:border-secondary-800 shadow-sm flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
        ...
        <div className="flex flex-col items-end">
          ...
          <div className="px-4 py-2 bg-slate-100 dark:bg-secondary-800 rounded-xl border border-slate-200 dark:border-secondary-700">
      ...
      {(stats?.balance || 0) < 10000 && currentPlan === "starter" && (
        <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 flex items-start gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {PLANS.map((plan) => {
          ...
          return (
            <div key={plan.id} className={`relative flex flex-col p-6 rounded-3xl bg-white dark:bg-secondary-900 border-2 transition-all duration-300 ${isCurrent ? 'border-primary-500 shadow-xl shadow-primary-500/10 ring-4 ring-primary-500/20' : plan.color}`}>
              {plan.badge && !isCurrent && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900 text-[10px] font-black rounded-full shadow-lg whitespace-nowrap">{plan.badge}</span>
              )}
              {isCurrent && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-primary-500 text-white text-[10px] font-black rounded-full shadow-lg whitespace-nowrap">Plan Actuel</span>
              )}
              ...
              <button 
                ...
                className={`w-full py-3 rounded-xl font-black text-xs transition-all ${
                  isCurrent 
                    ...
                    : "bg-slate-900 hover:bg-slate-800 dark:bg-primary-500 dark:hover:bg-primary-400 text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                }`}
              >
